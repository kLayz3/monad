...
class TAnalysisWorker final : public T {
	static_assert(mnd::has_process_entry<T>::value, "Type <T> needs a public `void ProcessEntry()` method implemented!");
...
private:
	std::thread _thread{};
	alignas(mnd::CL) std::atomic<bool> _stop{false};
	alignas(mnd::CL) std::atomic<bool> _has_work{false};
